...
const ChatScreen = ({ navigation, route }) => {
  ...
  const loadMessages = async () => {
    try {
      ...
      const mapped = res.data.map(msg => ({
        id: msg._id,
        text: msg.content,
        sender: msg.sender?._id === route.params?.currentUserId ? 'user' : 'other',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await api.post(`/chats/${chatId}/messages`, { content: newMessage.text });
    } catch (error) {
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerIcons}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('callScreen', { name, image, number })}>
  ...
  const MessageBubble = ({ text, sender, time }) => (
    <View
      ...
      <Text
        style={[
          styles.messageText,
          sender === 'other' ? styles.otherMessageText : styles.userMessageText,
        ]}>
        {text}
      </Text>
