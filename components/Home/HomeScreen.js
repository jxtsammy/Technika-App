...
import { Home, ClipboardList, MessageSquare, User, Bell } from 'lucide-react-native';
...
const HomeScreen = ({ navigation }) => {
  ...
  const [stats, setStats] = useState({ available: 0, completed: 0, pending: 0, averageCompletionMinutes: 0 });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabs}>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'quickstats' && { backgroundColor: '#007a3f' }]}
          onPress={() => setActiveTab('quickstats')}>
          <Text style={[styles.tabText, activeTab === 'quickstats' && { color: '#fff' }]}>
            Quick Stats
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tabButton, activeTab === 'map' && { backgroundColor: '#007a3f' }]}
          onPress={() => setActiveTab('map')}>
          <Text style={[styles.tabText, activeTab === 'map' && { color: '#fff' }]}>
            Map
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {activeTab === 'map' ? (
        <View style={styles.mapContainer}>
          <MapView
            ...
            {[
              { id: 1, lat: 6.31,   lng: 0.0565, title: 'Nearby Job 1' },
              { id: 2, lat: 6.312,  lng: 0.0525, title: 'Nearby Job 2' },
              { id: 3, lat: 6.305,  lng: 0.057,  title: 'GCB Bank' },
              { id: 4, lat: 6.3085, lng: 0.059,  title: 'Retail Store' },
              { id: 5, lat: 6.306,  lng: 0.051,  title: 'Construction Site' },
              { id: 6, lat: 6.3115, lng: 0.049,  title: 'School' },
              { id: 7, lat: 6.313,  lng: 0.054,  title: 'Hospital' },
            ].map((marker) => (
              <Marker
                key={marker.id}
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    {stat.change !== '' && (
                      <>
                        ...
                        <Text style={styles.statChangeText}> {stat.text}</Text>
                      </>
                    )}
                  </View>
  ...
  return (
    <>
      ...
      <Tab.Navigator
        screenOptions={({ route }) => ({
          tabBarIcon: ({ color }) => {
            if (route.name === 'Home')    return <Home size={30} color={color} />;
            if (route.name === 'Tasks')   return <ClipboardList size={30} color={color} />;
            if (route.name === 'Chats')   return <MessageSquare size={30} color={color} />;
            if (route.name === 'Profile') return <User size={30} color={color} />;
          },
          tabBarActiveTintColor: '#007a3f',
          tabBarInactiveTintColor: 'gray',
        })}>
        <Tab.Screen name="Home"    component={HomeScreen} options={{ headerShown: false }} />
        <Tab.Screen name="Tasks"   component={Tasks}      options={{ headerShown: false }} />
